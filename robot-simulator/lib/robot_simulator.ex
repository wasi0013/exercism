defmodule RobotSimulator do
  ...
  def create(direction \\ :north, position \\ {0, 0}) do
    cond do
      Enum.member?(@valid_directions, direction) == false ->  {:error, "invalid direction"}
      not_valid?(position) -> {:error, "invalid position"}
      true -> %{
        :direction => direction,
        :position => position,
      }
  ...
  def simulate(robot, "A"<>instructions) do
      case robot.direction do
        :east  -> Map.update!(robot, :position, fn {x,y} -> {x+1, y} end) |> simulate(instructions)
